...
docker run --rm --entrypoint /bin/bash "${image}" -lc '
  set -euo pipefail
  for command in labwc wlr-randr Xwayland sunshine pipewire wireplumber umu-run steam wine gamescope dwarfs firefox-esr foot thunar thunar-volman mousepad pavucontrol xfce4-terminal xfce4-session xfce4-panel xfdesktop xfsettingsd xfce4-display-settings; do
